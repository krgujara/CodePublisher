...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
	...
	try {
		...
		html_path = argv[argc - 1]; //last argument will be the html file path
	}
...
void CodeAnalysis::CodeAnalysisExecutive::setPatterns(std::string patt)
{
...
std::string CodeAnalysis::CodeAnalysisExecutive::getHtmlPath()
{
	return html_path;
}

//sets the path variable 
void CodeAnalysis::CodeAnalysisExecutive::setAnalysisPath(std::string pat)
{
	path_ = pat;
...
void CodeAnalysisExecutive::getSourceFiles()
{
	...
	std::cout << "Total files :" << numFiles_ << std::endl;
	for (auto patt : patterns_)
	{
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		std::unordered_map<Pattern, Files> files = exec.getFileMap();
